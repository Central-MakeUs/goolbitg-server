...
public class CronJobExecutor {
    ...
    public void sendChallengeAlarm() {
        ...
        for (User user : userRepository.findAll()) {
            ...
            List<ChallengeRecord> incompleted = challengeRecordRepository.findAllIncompletedRecords(userId, today);
            int totalSize = challengeRecordRepository.countByUserIdAndDate(userId, today);
            if (incompleted.size() > 0) {
                Challenge challenge = challengeRepository.findById(incompleted.get(0).getChallengeId())
